...
- **定时采集**：按话题配置 RSS 源，去重后增量入库；按源放宽时效窗口（慢更新源如 Cloudflare/Vercel）
- **AI 概述 + 排序**：对每篇文章生成要点概述，并按相关度 / 偏好 / 新颖度综合排序
- **关键词软信号**：关键词只做**加权**不硬过滤（低相关文章仍进库，靠排序压排名），避免好文因不命中关键词被误杀；概述 prompt 注入你的主题定位与长期画像，引导摘要贴合兴趣
- **每日新鲜池**：digest 只取最近 `FRESH_WINDOW_HOURS`（默认 24h）采集到的文章，昨天的自然滑出，评分/偏好只在今日新文内决定排序；空池回退全池
